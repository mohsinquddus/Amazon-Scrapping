...
def scrap(url):
    while True:
        try:
            ...
            with requests.get(url, stream=True,headers=headers, proxies={"http": f"http://hjqynrpv-{number}:s2nt3cxp4072@p.webshare.io:80/","https": f"http://hjqynrpv-{number}:s2nt3cxp4072@p.webshare.io:80/"}) as r:
    ...
    #                     print("IP ",r.raw._original_response.fp.raw._sock.getpeername()[0],'UN:',number)
                        if "To discuss automated access to Amazon data please contact" in r.text:
                            ...
                            print("Page %s must have been blocked by Amazon as the status code was %d"%(url,code))
                        raise Exception()
...
def get_asin(categories_all_pages):
    ASIN=[]
    for index,page in enumerate(categories_all_pages):
        sleep(2)
        soup,IP,UN=scrap(page)
        ...
        if not os.path.exists('ASIN.csv'):
            pd.DataFrame(columns=['ASIN']).to_csv("ASIN.csv",index=None)
        ...
        while l1==l2:
            sleep(2)
            print("Same Result on:",IP,"Proxy_Number",UN)
            soup,IP,UN=scrap(page)
            divs=soup.find('div',{'class':'s-main-slot s-result-list s-search-results sg-row'})
            for div in divs.prettify().split("data-asin"):
                if div[2:12].isupper():
...
while True:
    ...
    get_asin(pages_to_scrap)
    batch_size+=20
    batches+1
    with open('Batches.txt', 'wb') as handle:
        pickle.dump(batches, handle, protocol=pickle.HIGHEST_PROTOCOL)
    print(f"Batch {batches} Complete of Batch Size 20",)
get_asin(categories_all_pages)
